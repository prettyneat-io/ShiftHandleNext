  ...
  <div class="space-y-6">
    <header class="mb-6 border-b border-neutral-light dark:border-neutral-dark pb-3 flex flex-wrap items-center justify-between gap-4">
      ...
      <div class="flex items-center gap-3">
        <AppButton @click="openCreateModal" variant="primary" size="small">
          Create
        </AppButton>
...
>
  ...
  <ChevronDownIcon class="-mr-1 h-5 w-5 text-neutral" aria-hidden="true" />
    ...
    <AppModal :isOpen="isCreateModalOpen" @close="closeCreateModal">
      <template #title>New {{ title.slice(0) }}</template>
      <template #content>
        <DynamicCreateForm
          :endpoint="endpoint"
          :fields="columns"
          :title="`New ${title.slice(0)}`"
          :subtitle="`Create a new ${title.slice(0).toLowerCase()} record`"
          :cacheInvalidateKey="`list:${endpoint}`"
          mode="create"
...
import AppModal from '../../../layers/shared/components/AppModal.vue'
import DynamicCreateForm from '../../../layers/shared/components/forms/DynamicCreateForm.vue'
import FormsFormTable, { type Col } from '../../../layers/shared/components/forms/FormTable.vue'
import FormsCardGrid from '../../../layers/shared/components/forms/CardGrid.vue'
...
const q = ref('')
const debouncedQ = refDebounced(q, 300) // Debounce search input
...
const formTableRef = ref<InstanceType<typeof FormsFormTable> | InstanceType<typeof FormsCardGrid> | null>(null)
...
const searchableColumns = computed(() => {
  ...
  return props.columns
    ...
    .map(c => ({
      // Use displayPath for the value if it exists, otherwise fall back to the key
      key: c.displayPath || c.key,
      label: c.label,
    }));
});

// Computed property for the search input's placeholder
...
onMounted(() => {
  ...
  // Initially, make all columns visible
