...
const setColorVariables = (themeKey: ThemeKey) => {
  ...
  ;(Object.entries(theme.colors) as Array<[
    ...
    root.style.setProperty(`--theme-${token}`, toRgb(variants.DEFAULT))
    root.style.setProperty(`--theme-${token}-light`, toRgb(variants.light))
    root.style.setProperty(`--theme-${token}-dark`, toRgb(variants.dark))
  })
